...
function AnimatedCounter({ target, prefix = '', suffix = '' }: { target: number; prefix?: string; suffix?: string }) {
  ...
  return (
    <span className="tabular-nums">
      {prefix}{count < 10 ? count.toFixed(1) : Math.round(count).toLocaleString()}{suffix}
    </span>
  );
}
      ...
      <div className="p-4 max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Layers className="h-5 w-5 text-[#F0B90B]" />
            <h1 className="text-lg font-bold text-[#EAECEF]">{t('defiDashboard.title')}</h1>
          </div>
          <Badge className="bg-[#0ECB81]/10 text-[#0ECB81] border-0 text-[10px] font-semibold px-2 flex items-center gap-1">
            <div className="w-1.5 h-1.5 rounded-full bg-[#0ECB81] pulse-green" />
            {t('status.live')}
          </Badge>
        </div>
        ...
        >
          <Card className="bg-gradient-to-br from-[#1E2329] to-[#2B3139] border-[#2B3139] overflow-hidden relative">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#F0B90B]/5 rounded-full -translate-y-1/3 translate-x-1/3" />
            <div className="absolute bottom-0 left-0 w-20 h-20 bg-[#0ECB81]/5 rounded-full translate-y-1/3 -translate-x-1/3" />
            <CardContent className="p-5 relative z-10">
              <p className="text-xs text-[#5E6673] uppercase tracking-wider mb-1">{t('defiDashboard.totalValueLocked')}</p>
              <div className="flex items-end gap-3">
                <h2 className="text-3xl font-bold text-[#EAECEF] animate-count-up">
                  <AnimatedCounter target={187.4} prefix="$" suffix="B" />
                </h2>
                <span className="text-sm font-semibold text-[#0ECB81] flex items-center gap-0.5 mb-1">
        ...
        {/* Top Protocols */}
        <div>
          <h3 className="text-sm font-semibold text-[#EAECEF] mb-3">{t('defiDashboard.topProtocols')}</h3>
                ...
                <React.Fragment key={protocol.name}>
                  <div className="flex items-center justify-between px-4 py-3 hover:bg-[#2B3139]/30 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <Button className="h-7 px-3 text-[10px] bg-[#2B3139] hover:bg-[#3B4451] text-[#EAECEF] border-0 press-scale">
                        {t('actions.deposit')}
                      </Button>
                    </div>
                  </div>
                  {i < filteredProtocols.length - 1 && <div className="border-t border-[#2B3139] mx-4" />}
                </React.Fragment>
              ))}
            </CardContent>
          </Card>
        ...
        {/* My DeFi Positions */}
        <div>
          <h3 className="text-sm font-semibold text-[#EAECEF] mb-3">{t('defiDashboard.myPositions')}</h3>
          <div className="space-y-3">
            {myPositions.map((pos, i) => (
              ...
              >
                <Card className="bg-[#1E2329] border-[#2B3139] hover-lift">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[#EAECEF]">{pos.protocol}</p>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#2B3139]">
                      <span className="text-[10px] text-[#5E6673]">{pos.rate}</span>
                      <Button variant="outline" className="h-7 px-3 text-[10px] border-[#F6465D]/30 text-[#F6465D] hover:bg-[#F6465D]/10 press-scale">
        ...
        <div>
          <h3 className="text-sm font-semibold text-[#EAECEF] mb-3">{t('defiDashboard.yieldComparison')}</h3>
          <Card className="bg-[#1E2329] border-[#2B3139]">
            <CardContent className="p-4">
              <div className="space-y-3">
                {yieldData.map((item) => (
                  <div key={item.name} className="flex items-center gap-3">
                    <span className="text-xs text-[#848E9C] w-20 shrink-0">{item.name}</span>
                    <div className="flex-1 h-6 bg-[#2B3139] rounded overflow-hidden relative">
                      <motion.div
              ...
              <p className="text-[10px] text-[#5E6673] mt-3">{t('defiDashboard.apyLabel')}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div>
          ...
          <Card className="bg-[#1E2329] border-[#2B3139]">
            <CardContent className="p-4 flex flex-col items-center">
              <RiskGauge value={65} />
              <p className="text-xs text-[#848E9C] mt-2 text-center">{t('defiDashboard.riskDescription')}</p>
            </CardContent>
          </Card>
        </div>
