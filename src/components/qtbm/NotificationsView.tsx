...
const typeConfig: Record<string, { icon: React.ElementType; color: string; bgColor: string; stripeClass: string }> = {
  security: { icon: Shield, color: 'text-[#F6465D]', bgColor: 'bg-[#F6465D]/10', stripeClass: 'notif-stripe-security' },
  trade: { icon: TrendingUp, color: 'text-[#0ECB81]', bgColor: 'bg-[#0ECB81]/10', stripeClass: 'notif-stripe-trade' },
  deposit: { icon: ArrowDownRight, color: 'text-[#F0B90B]', bgColor: 'bg-[#F0B90B]/10', stripeClass: 'notif-stripe-deposit' },
  withdrawal: { icon: ArrowUpRight, color: 'text-[#3B82F6]', bgColor: 'bg-[#3B82F6]/10', stripeClass: 'notif-stripe-withdrawal' },
  system: { icon: Settings, color: 'text-[#848E9C]', bgColor: 'bg-[#848E9C]/10', stripeClass: 'notif-stripe-system' },
  promotion: { icon: Gift, color: 'text-[#8B5CF6]', bgColor: 'bg-[#8B5CF6]/10', stripeClass: 'notif-stripe-promotion' },
};
  ...
  return (
    <ScrollArea className="h-[calc(100vh-8rem)] lg:h-[calc(100vh-4rem)]">
      <div className="p-4 space-y-4 max-w-4xl mx-auto">
        ...
        <div className="relative flex gap-1 bg-[#1E2329] rounded-lg p-1 overflow-x-auto">
          {filterTabs.map((tab) => (
            <button
              key={tab.id}
            ...
            >
              ...
              {activeFilter === tab.id && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-1 right-1 h-0.5 bg-[#F0B90B] rounded-full"
        ...
        {/* Notification List */}
        <div className="space-y-1 fancy-scrollbar max-h-[60vh] overflow-y-auto">
            ...
            <Card className="bg-[#1E2329] border-[#2B3139]">
              <CardContent className="p-8 text-center">
                <motion.div
                  ...
                  className="mx-auto mb-3 w-16 h-16 rounded-full bg-[#0ECB81]/10 flex items-center justify-center"
                >
            ...
            filteredNotifications.filter(n => !dismissedIds.has(n.id)).map((notification, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative w-full rounded-lg transition-all hover:bg-[#1E2329] card-hover-effect ${
                    !notification.isRead ? `bg-[#1E2329]/80 ${config.stripeClass}` : ''
                  }`}
                >
                  <button
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p
                          className={`text-sm font-semibold ${
                            notification.isRead ? 'text-[#848E9C]' : 'text-[#EAECEF]'
                          }`}
                        >
                          {notification.title}
                        </p>
                        {!notification.isRead && (
                          <div className="w-2 h-2 rounded-full bg-[#F0B90B] shrink-0 pulse-badge" />
                        )}
                      </div>
                      <p
                        className={`text-xs mt-0.5 ${
                          notification.isRead ? 'text-[#5E6673]' : 'text-[#848E9C]'
                        }`}
                      >
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-[9px] h-4 px-1.5 border-0 ${config.bgColor} ${config.color}`}
                        >
                          {notification.type}
                        </Badge>
                      </div>
                    ...
                    <button
                      ...
                      className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[#5E6673] hover:text-[#EAECEF] hover:bg-[#2B3139] transition-all"
                    >
        ...
        {/* Settings Link */}
        <Card className="bg-[#1E2329]/50 border-[#2B3139] cursor-pointer hover:bg-[#1E2329] transition-colors"
